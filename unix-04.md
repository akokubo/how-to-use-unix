...
これまで見てきたように、`cp` や `mv` コマンド、そしてリダイレクト `>` は、コピー先や変更後のファイル名、リダイレクト先のファイルが既に存在する場合、**ユーザーに確認することなくファイルを上書きしてしまう。** これは、WindowsなどのGUI操作での「ファイルの置換またはスキップ」という確認ダイアログに慣れていると、非常に危険な動作に感じられるだろう。慣れないうちは、この仕様が原因で誤って重要なファイルを消してしまうかもしれない。

![Windowsの「ファイルの上書きの確認」メッセージ](images/overwrite.png)
