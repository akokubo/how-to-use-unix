...
最初に UNIX システムの全体像を紹介してしまおう。
次のページの図 \ref{dirstru} は、 UNIX のディレクトリ構造である。
なお、主なものだけしか描いてない。
...
一番上に、 \texttt{/} と書かれるルート・ディレクトリと呼ばれるものがある。
「ルート -- root 」というのは「(木の)根」のことだ。
そして、その下に \texttt{home}、 \texttt{bin} 、 \texttt{etc} など、
...
つまり、 \texttt{kokubo} さんのホーム・ディレクトリは、 \texttt{/} の下の
 \texttt{home} の下の \texttt{kokubo} だ。
なおこれは、 \texttt{/home/kokubo} と表す。
要するに、ディレクトリの区切りマークが「\texttt{/}」というわけだ。

また同様に、 \texttt{tomoda} さんのホーム・ディレクトリは、 \texttt{/} の下の
 \texttt{home} の下の \texttt{tomoda} で、
この場所のことを \texttt{/home/tomoda} と表す。

 login した直後、それぞれのユーザは、自分のホーム・ディレクトリに、まず入る。
ここまでの演習で作ったファイルも、みんなここにある。

例えば、 \texttt{kokubo} さんは、 login すると、 \texttt{/home/kokubo} に入る。
...
本来の意味ではバイナリである。
 UNIX システムでは、これらの内、コマンドを入れるためのディレクトリに
 \texttt{bin} を使う。
...
\item \textbf{\large etc}
 ...
 UNIX システムでは、ここにシステムの各種設定用のファイルが置かれている。
...
置かれている。
これらは、 UNIX の基本システムには付いて来ないものだが、
便利なものも多い。
例えば、 Mule などもここに置かれている。
...
\item \textbf{\large var}
 ...
 login の記録や、メールなどのシステムの記録などの、日々変化するものが置かれている。
\end{itemize}
...
B 演習室では、ホーム・ディレクトリの様子は、図 \ref{Benshu} のようになっている。
...
その中に \texttt{ei99000} さんのホーム・ディレクトリがあって、
これは \texttt{/home/ei99/ei99000} だ。

たとえば、 \texttt{ei99000} さんは、自分のホーム・ディレクトリの中に
 \texttt{Report} というディレクトリを作ることができて、
これは \texttt{/home/ei99/ei99000/Report} になる。

そして、またその中にディレクトリやファイルを作ることもできる。
...
この下に、 \texttt{Report} というディレクトリがある。
これは、相対パスでは、単に \texttt{Report} である。
絶対パスだと、わざわざ \texttt{/home/ei99/ei99000/Report} と長くて面倒だが、
相対パスではこんなに短くなる。

それから、 \texttt{Report} の下の \texttt{11gatsu} も、
 \texttt{Report/11gatsu} ととても簡単になる。
...
つまり、自分のホームの一つ上 \texttt{/home/ei99} は、単に \texttt{..} となる。
...
同様にルート・ディレクトリ \texttt{/} は、相対パスでは \texttt{../../..} である。
...
つまり、今 \texttt{/home/ei99/ei99000} にいるなら、ここは相対パスで
表すと \texttt{.} となる。
...
例えば、 \texttt{/home/ei99/ei99000/Report} にいる場合、
自分のホーム・ディレクトリは一つ上にあるので、相対パスでは \texttt{..} となる。
 \texttt{11gatsu} は、今いるところのすぐ下にあるので、相対パスでは
 \texttt{11gatsu} になる。
...
\end{progls}
 ...
 \texttt{ei99000} さんの場合、今いるのは、 \texttt{/home/ei99/ei99000} だ。

なお、現在の状況は図 \ref{now} のようになっているものとしておく。
...
現在、図 \ref{now} のように、 \texttt{/home/ei99/ei99000} にいる。
 1 つ上のディレクトリは、相対パスでは \texttt{..} で、
絶対パスでは \texttt{/home/ei99}だ。

 1 つ上に移動するには、 \texttt{cd ..} でも、
\texttt{cd /home/ei99} でも、どちらでもいい。
とは言え、明らかに \texttt{cd ..} の方が楽なので、こちらを実行してみよう。

\begin{figure}[H]
\centering{\includegraphics[scale=0.8]{abrel.eps}}
\caption{現在いるディレクトリと、関係するパス}\label{now}
\end{figure}
...
次は、絶対パスを使って、一気に \texttt{/bin} ディレクトリに移動してみよう。
...
と、確かに \texttt{/bin} にいることがわかる。
どんなファイルがあるか \texttt{ls} で見てみよう。
...
いつ、どこにいても、自分のホーム・ディレクトリには、 \texttt{cd} だけで戻れる。
...
では、自分のホームの下に {\tt Enshu} とかいうディレクトリを作ってみよう。
...
これでは、 \texttt{Enshu} はあるが、ファイルかディレクトリか区別が
つかないので、 \texttt{ls -F} してみよう。
...
今、ホーム・ディレクトリは一つ上なので、相対パスでは \texttt{..} 。
また、絶対パスでは \texttt{/home/ei99/ei99000} になる。

よってこの場合、相対パスで \texttt{cd ..} と打った方が簡単だ。
しかし、単にホームに戻るには \texttt{cd} なので、これでもいい。
今回はこちらを使おう。
...
例えば、 \texttt{ThisMonth} というファイルを \texttt{CAL} という
...
ファイルを移動やコピーする場合でも、 \texttt{mv} や \texttt{cp} の使い方は、
普通に移動したりコピーする場合と全く同じだ。
ただ、ファイル名として、パスを書いてやればいい。

つまり、 \texttt{ThisMonth} を \texttt{CAL} に移動するには、
\texttt{mv ThisMonth CAL/ThisMonth} である。

なお、これは、 \texttt{mv ThisMonth CAL/.} とも書ける。
「\texttt{.}」をファイル名の代わりに使うと、「同じ名前で」という意味になるのだ。
...
なお、これは \texttt{mv} を使った例だが、代わりに \texttt{cp} を使えば、
コピーになる。
\newpage
{~}
