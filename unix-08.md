...
現在、macOSのターミナルを起動するとデフォルトで`zsh` が、WSL上のUbuntuなど多くのLinux環境では`bash` が起動することが一般的である。これらのシェルは、ターミナルでその名前（例: `bash`、`zsh`）を入力することで一時的に切り替えて試すこともできる。
...
*   **`bash`**: `~/.bashrc` (対話的シェルが起動するたびに読み込まれる), `~/.bash_profile` (ログイン時に読み込まれる)
*   **`zsh`**: `~/.zshrc` (対話的シェルが起動するたびに読み込まれる), `~/.zprofile` (ログイン時に読み込まれる)
...
**`bash` (バッシュ) ユーザーの場合**: `~/.bashrc` に以下のように記述（追記）する。
```sh
# My Custom Aliases
alias ls='ls -F'
...
**`zsh` (ゼットエスエイチ) ユーザーの場合**: `~/.zshrc` に同様に記述（追記）する。
```sh
# My Custom Aliases
alias ls='ls -F'
alias ll='ls -lFh'
