...
例えば、カレントディレクトリにある `.py` で終わるすべてのC言語ソースファイルを表示したい場合は、以下のようにする。
...
**注意**: `rm` (アールエム) コマンドとワイルドカードを組み合わせる際は、意図しないファイルを削除してしまわないよう、事前に `ls` コマンドで対象ファイルを確認するなど、慎重に操作すること。特に `rm *` のようなコマンドは非常に危険である。
...
それぞれファイル名の先頭にある `.` は、そのファイルが隠しファイルであることを意味する。`ls -a` (エルエス エー) コマンドで表示できる。

### 設定ファイルへのエイリアスの記述例

例えば、`ls -F` (エルエス エフ) のエイリアスを `ls` (エルエス) として永続的に設定したい場合、

** WSL上のUbuntu(`bash`): `~/.bashrc` に以下のように記述（追記）する。
```sh
alias ls='ls -F'
```

** macOS(`zsh`): `~/.zshrc` に同様に記述（追記）する。
```sh
alias ls='ls -F'
```
