...
例えば、WSL上のUbuntuの例では、`Linux` がカーネル名、`5.15.167.4-microsoft-standard-WSL2` がそのバージョン、`x86_64` がCPUアーキテクチャ（一般的なIntel/AMD系CPU）を示している。

macOSの例では、`Darwin` がOSのカーネル（核）の名前、`24.4.0` がそのバージョン、`arm64` がCPUアーキテクチャ（Apple Siliconなど）を示している。
...
*   **`command not found` エラー**: コマンド名が間違っていたり、システムにそのコマンドがインストールされていなかったりすると、「command not found」（コマンドが見つかりません）のようなエラーメッセージが表示される。まずはコマンドのスペルを確認しよう。

WSL上のUbuntu
```sh
unmae -a
Command 'unmae' not found, did you mean:
